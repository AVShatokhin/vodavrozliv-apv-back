module.exports = (config) => {
  return {
    getAPVconfig: `SELECT sn, address, tgLink, snEQ FROM apv`,
    kvsReplace: `REPLACE INTO kvs SET link=?, value=?`,
    kvsGet: `SELECT value FROM kvs WHERE link=?;`,
    appendMain: `INSERT INTO main (sn, FLAG_start, version, w, k, r, m, m1, m2, m5, m10, c, errorDevice, errorCode, messCode, FLAG_k_off, FLAG_r_off, FLAG_m_off, FLAG_c_off) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    updateApv: `UPDATE apv SET version=?, cost=?, phone=?, linkState=?, oper=?, lts=now() WHERE sn=?`,
    appendInkas: `INSERT INTO inkas (sn, inkas_number, date, version, inkas, kup, box, rd, op, op_extended, op_state) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
